      ...
      <div className="max-w-5xl mx-auto p-4 md:p-8">
            ...
            <div className="flex items-center justify-center gap-4 md:gap-8 my-10">
              ...
              <div className="w-full max-w-sm h-96 cursor-pointer" style={{ perspective: '1000px' }} onClick={() => setIsFlipped(!isFlipped)}>
                <div className="w-full h-full transition-transform duration-500 relative" style={{ transformStyle: 'preserve-3d', transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
                  ...
                  {/* קדמי */}
                  <div className="absolute inset-0 bg-gradient-to-br from-[#7e22ce] to-[#3b82f6] rounded-[40px] shadow-[0_10px_40px_rgb(59,130,246,0.3)] flex flex-col justify-center items-center p-8 text-white text-center border-4 border-white/20" style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden' }}>
                    <button onClick={(e) => { e.stopPropagation(); speakWord(getFilteredWords()[currentIndex].english); }} className="absolute top-6 left-6 w-14 h-14 bg-white/20 hover:bg-white/40 rounded-2xl flex justify-center items-center text-2xl transition-colors active:scale-90">🔊</button>
                    <h3 className="text-5xl font-black tracking-tight mb-4 drop-shadow-lg">{getFilteredWords()[currentIndex].english}</h3>
                    <p className="text-white/60 font-bold uppercase tracking-widest text-sm mt-4 bg-white/10 px-4 py-2 rounded-full">לחצי להפוך</p>
                  </div>

                  {/* אחורי */}
                  <div className="game-card absolute inset-0 flex flex-col justify-center items-center p-8 text-center" style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
                    <span className="text-purple-500 font-black tracking-widest text-xs uppercase mb-4 bg-purple-50 px-3 py-1 rounded-full">התרגום לעברית</span>
                    <h3 className="text-4xl font-black text-slate-800 mb-8 leading-tight">{getFilteredWords()[currentIndex].hebrew}</h3>
                    <button onClick={(e) => { e.stopPropagation(); speakWord(getFilteredWords()[currentIndex].english); }} className="btn-3d-white text-lg w-full max-w-[200px]" dir="ltr">
        ...
        {page === 'exam' && examWords.length > 0 && !examFinished && (
          <div className="space-y-10 text-center max-w-2xl mx-auto animate-fade-in">
            ...
            <div className="game-card h-80 flex flex-col justify-center items-center p-8 cursor-pointer group" onClick={() => setIsFlipped(!isFlipped)}>
              {!isFlipped ? (
                <>
                  <h3 className="text-6xl font-black text-slate-800 mb-8">{examWords[currentIndex].english}</h3>
                  <p className="text-slate-400 font-black bg-slate-50 px-6 py-3 rounded-2xl group-hover:bg-slate-100 transition-colors">לחצי כדי לחשוף את התשובה 👀</p>
                </>
              ) : (
                <>
                  <span className="text-sm font-black text-[#58cc02] mb-4 bg-[#58cc02]/10 px-4 py-1.5 rounded-full uppercase tracking-wider">התרגום הנכון</span>
                  <h3 className="text-5xl font-black text-slate-800">{examWords[currentIndex].hebrew}</h3>
                </>
              )}
            </div>
