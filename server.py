...
def seed_data():
   # --- יחידה 1 ---
    ...
    if not u1:
        ...
        db.session.commit()
        w1 = [
            ("10th - tenth", "עשירי"), ("1st - first", "ראשון"), ("2nd - second", "שני"), ("3rd - third", "שלישי"),
            ("4th - fourth", "רביעי"), ("5th - fifth", "חמישי"), ("6th - sixth", "שישי"), ("7th - seventh", "שביעי"),
            ("8th - eighth", "שמיני"), ("9th - ninth", "תשיעי"), ("about", "אודות / על"), ("about", "בערך / סביב"),
            ("above", "מעל"), ("above all", "מעל הכל / בראש ובראשונה"), ("absolutely", "לחלוטין / בהחלט"), ("according", "לפי / בהתאם ל-"),
            ("accordingly", "בהתאם לכך"), ("across", "מעבר ל- / ברחבי"), ("additionally", "בנוסף לכך"), ("adjacent to", "סמוך ל- / ליד"),
            ("after", "אחרי / לאחר"), ("after all", "אחרי הכל"), ("after all", "למרות הכל"), ("afterwards", "לאחר מכן"),
            ("again", "שוב"), ("against", "נגד / כנגד"), ("ahead of", "לפני (בזמן/מקום)"), ("albeit", "אם כי / אף על פי ש-"),
            ("all in all", "בסך הכל"), ("along", "לאורך"), ("along with", "יחד עם"), ("alongside", "לצד / במקביל"),
            ("also", "גם / בנוסף"), ("although", "למרות ש-"), ("altogether", "לגמרי / בסך הכל"), ("always", "תמיד"),
            ("amid", "בקרב / בתוך"), ("among", "בין (רבים) / בקרב"), ("and", "ו- / וגם"), ("around", "מסביב / בערך"),
            ("as", "כמו / בתור"), ("as", "מכיוון ש- / כש-"), ("as a result", "כתוצאה מכך"), ("as if", "כאילו"),
            ("as long as", "כל עוד"), ("as soon as", "ברגע ש- / בהקדם"), ("as well as", "כמו גם / בנוסף ל-"), ("aside from", "מלבד / חוץ מ-"),
            ("at", "ב- / אצל / בתוך"), ("at first", "בהתחלה"), ("at last", "לבסוף"), ("backward", "אחורה / לאחור"),
            ("based on", "מבוסס על"), ("because", "בגלל ש- / כי"), ("because of", "בגלל / בעקבות"), ("before", "לפני"),
            ("before", "בטרם"), ("behind", "מאחורי"), ("being that", "היות ש- / מכיוון ש-"), ("below", "מתחת"),
            ("beneath", "מתחת ל- / למטה מ-"), ("beside", "לצד / על יד"), ("besides", "חוץ מזה / מלבד"), ("besides", "בנוסף ל-"),
            ("besides", "יתר על כן"), ("between", "בין (שניים)"), ("beyond", "מעבר ל-"), ("both", "שניהם / שתי-"),
            ("both... and…", "גם... וגם..."), ("but", "אבל"), ("but", "אלא / פרט ל-"), ("by", "על ידי / באמצעות"),
            ("by", "עד (זמן) / ליד"), ("certainly", "בוודאות / בהחלט"), ("chiefly", "בעיקר"), ("close", "קרוב / סמוך"),
            ("comparatively", "באופן יחסי"), ("consequently", "כתוצאה מכך"), ("consistently", "בעקביות / באופן עקבי"), ("constantly", "כל הזמן / בתמידות"),
            ("contradictory", "סותר / נוגד"), ("contrary", "מנוגד / ההפך"), ("conversely", "להפך / מאידך גיסא"), ("correspondingly", "בהתאמה"),
            ("coupled with", "בשילוב עם"), ("currently", "כעת / כרגע"), ("daily", "יומי / מדי יום"), ("despite", "למרות / על אף"),
            ("different from", "שונה מ-"), ("distant", "רחוק"), ("down", "למטה"), ("due to", "עקב / בגלל"),
            ("during", "במהלך / במשך"), ("e.g.", "לדוגמה (למשל)"), ("earlier", "מוקדם יותר"), ("either... or…", "או... או..."),
            ("elsewhere", "במקום אחר"), ("equally", "במידה שווה"), ("especially", "במיוחד"), ("essentially", "בעיקרו של דבר / ביסודו"),
            ("even if", "אפילו אם"), ("even so", "אף על פי כן / למרות זאת"), ("even though", "אף על פי ש- / למרות ש-"), ("eventually", "בסופו של דבר"),
            ("every day", "כל יום"), ("every now and then", "מדי פעם"), ("except", "חוץ מ- / פרט ל-"), ("farther", "רחוק יותר (פיזית)"),
            ("finally", "לבסוף"), ("first of all", "קודם כל / ראשית"), ("for", "בשביל / עבור"), ("for", "כי / מכיוון ש- / במשך (זמן)"),
            ("for example", "לדוגמה"), ("for instance", "למשל"), ("for the most part", "לרוב / בדרך כלל"), ("forever", "לתמיד / לעד"),
            ("former", "קודם / לשעבר"), ("forward", "קדימה"), ("frequently", "לעיתים קרובות"), ("from", "מ- / מתוך"),
            ("from", "מאת"), ("from time to time", "מדי פעם"), ("further", "הלאה / עוד"), ("further", "נוסף / מעבר לכך"),
            ("furthermore", "יתרה מכך / זאת ועוד"), ("given that", "בהתחשב בכך ש-"), ("granted (that)", "אמנם / בהנחה ש-"), ("hardly ever", "כמעט אף פעם לא"),
            ("hence", "לפיכך / משום כך"), ("here", "כאן / פה"), ("how", "איך / כיצד"), ("however", "אולם / יחד עם זאת"),
            ("however", "בכל דרך ש-"), ("i.e.", "כלומר / דהיינו"), ("identically", "באופן זהה"), ("If", "אם"),
            ("immediately", "מיד / באופן מיידי"), ("in", "ב- / בתוך"), ("in a moment", "בעוד רגע / מיד"), ("in a word", "במילה אחת"),
            ("in addition", "בנוסף / יתר על כן"), ("in case", "במקרה ש- / למקרה ש-"), ("in comparison", "בהשוואה"), ("in conclusion", "לסיכום"),
            ("in contrast to", "בניגוד ל-"), ("in due time", "בבוא הזמן"), ("in fact", "למעשה"), ("in fact", "בעצם"),
            ("in front of", "בפני / מקדימה / לפני"), ("in general", "באופן כללי"), ("in light of", "לאור / נוכח"), ("in order to", "כדי / על מנת"),
            ("in other words", "במילים אחרות"), ("in particular", "במיוחד / בפרט"), ("in spite of", "למרות / על אף"), ("in that case", "במקרה כזה"),
            ("in the same way", "באותו אופן"), ("incidentally", "דרך אגב"), ("including", "כולל"), ("indeed", "אכן / באמת"),
            ("initially", "בתחילה / מלכתחילה"), ("inside", "בפנים / בתוך"), ("instantly", "מיידית / בן רגע"), ("instead", "במקום זאת"),
            ("instead of", "במקום"), ("into", "לתוך / אל תוך"), ("just as", "בדיוק כמו ש-"), ("just as… so…", "כשם ש... כך..."),
            ("last", "אחרון / שעבר"), ("later", "מאוחר יותר"), ("latter", "השני מבין שניים / האחרון"), ("led to", "הוביל ל-"),
            ("left", "שמאל"), ("like", "כמו"), ("likewise", "כמו כן / באופן דומה"), ("mainly", "בעיקר"),
            ("markedly", "באופן בולט / ניכר"), ("meanwhile", "בינתיים"), ("moreover", "יתרה מכך / בנוסף"), ("mostly", "לרוב / בעיקר"),
            ("namely", "כלומר / דהיינו"), ("near", "קרוב"), ("nearby", "בקרבת מקום / סמוך"), ("neither... nor…", "לא... ולא..."),
            ("never", "לעולם לא / אף פעם"), ("nevertheless", "אף על פי כן / למרות זאת"), ("next", "הבא / אחר כך"), ("next to", "ליד / סמוך ל-"),
            ("nonetheless", "אף על פי כן"), ("not only", "לא רק"), ("not only... but also…", "לא רק... אלא גם..."), ("not to mention", "שלא לדבר על"),
            ("not... but…", "לא... אלא..."), ("notably", "במיוחד / ראוי לציון"), ("now", "עכשיו"), ("occasionally", "מדי פעם / לעיתים"),
            ("of", "של / מתוך"), ("of course", "כמובן"), ("of course… but…", "כמובן ש... אבל..."), ("often", "לעיתים קרובות"),
            ("on", "על / ב- (תאריכים/ימים)"), ("on one hand…on the other hand", "מצד אחד... מצד שני"), ("on the contrary", "נהפוך הוא / להפך"), ("on top of", "מעל / בנוסף ל-"),
            ("once", "פעם אחת / בעבר"), ("once", "ברגע ש- / כאשר"), ("once in a while", "מדי פעם"), ("only", "רק / בלבד"),
            ("opposite", "מול / ממול / ההפך"), ("or", "או"), ("ordinarily", "בדרך כלל / ברגיל"), ("otherwise", "אחרת"),
            ("outside", "בחוץ"), ("outside", "מחוץ ל-"), ("over", "מעל / על פני"), ("overall", "באופן כללי / כולל"),
            ("owing to", "בגלל / הודות ל-"), ("particularly", "במיוחד / בפרט"), ("past", "מעבר ל- / עבר"), ("permanently", "לצמיתות / קבוע"),
            ("perpetually", "לנצח / בתמידות"), ("presently", "כעת / בקרוב"), ("previously", "קודם לכן / בעבר"), ("primarily", "בראש ובראשונה / בעיקר"),
            ("prior to", "לפני / טרם"), ("provided that", "בתנאי ש-"), ("rarely", "לעיתים נדירות"), ("rather", "למדי / די / אלא"),
            ("rather than", "מאשר / במקום"), ("regardless", "ללא קשר / ללא התחשבות"), ("regularly", "באופן קבוע"), ("right", "ימין / צודק / מיד"),
            ("seldom", "לעיתים נדירות"), ("shortly", "בקרוב / זמן קצר לאחר מכן"), ("sideways", "הצידה"), ("significantly", "באופן משמעותי"),
            ("similarly", "באופן דומה"), ("simultaneously", "בו זמנית"), ("since", "מאז / מכיוון ש-"), ("since", "מכיוון ש-"),
            ("so", "לכן / אז / כל כך"), ("so long as", "כל עוד"), ("so that", "כדי ש- / על מנת ש-"), ("sometimes", "לפעמים"),
            ("soon", "בקרוב"), ("specifically", "ספציפית / בייחוד"), ("still", "עדיין / בכל זאת"), ("subsequently", "לאחר מכן / כתוצאה מכך"),
            ("such as", "כגון / כמו"), ("suddenly", "פתאום"), ("surely", "בוודאי / לבטח"), ("surprisingly", "באופן מפתיע"),
            ("than", "מאשר / מ- (השוואה)"), ("thanks to", "הודות ל-"), ("that", "ש- / הזה / ההוא"), ("then", "אז / באותו זמן"),
            ("then", "לאחר מכן / אז"), ("there", "שם"), ("therefore", "לכן / לפיכך"), ("though", "למרות ש- / אף על פי ש-"),
            ("though", "אבל / עם זאת (בסוף משפט)"), ("thus", "לפיכך / כך"), ("to begin with", "קודם כל / בתור התחלה"), ("to sum up", "לסיכום"),
            ("together with", "יחד עם"), ("too", "גם / יותר מדי"), ("too", "מדי (לפני תואר)"), ("toward", "לעבר / לכיוון"),
            ("truly", "באמת / בכנות"), ("under", "מתחת ל- / תחת"), ("underneath", "מתחת / למטה"), ("undoubtedly", "ללא ספק"),
            ("unless", "אלא אם כן"), ("unlike", "בניגוד ל- / שונה מ-"), ("until", "עד / עד ש-"), ("until now", "עד עכשיו"),
            ("up", "למעלה"), ("upon", "על / בעת / מיד לאחר"), ("upside down", "הפוך / ראש למטה"), ("usually", "בדרך כלל"),
            ("vice versa", "להפך / וההפך"), ("what", "מה / איזה"), ("when", "מתי / כאשר"), ("whenever", "בכל פעם ש- / מתי ש-"),
            ("where", "איפה / היכן"), ("where", "המקום שבו"), ("whereas", "ואילו / בעוד ש-"), ("wherever", "בכל מקום ש- / לאן ש-"),
            ("whether... or…", "בין אם... ובין אם..."), ("which", "איזה / אשר / ש-"), ("which", "הדבר ש-"), ("while", "בזמן ש- / בעוד ש- / למרות ש-"),
            ("while", "פרק זמן"), ("who", "מי / אשר / ש- (לאנשים)"), ("whom", "אותו / למי / אשר (מושא לאנשים)"), ("whose", "של מי / אשר שלו"),
            ("why", "למה / מדוע"), ("with", "עם / באמצעות"), ("within", "בתוך / במסגרת (זמן/מקום)"), ("without", "בלי / ללא"),
            ("yet", "עדיין (בשאלה/שלילה)"), ("yet", "אולם / עם זאת")
        ]
